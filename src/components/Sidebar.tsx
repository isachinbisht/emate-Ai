...
import { getSubjects, addSubject, deleteSubject, renameSubject, getNotebook, Subject } from '@/lib/notebook';
...
export default function Sidebar({ onToggle, width = 248, onOpenSettings, onOpenNotebook }: SidebarProps) {
        ...
        <div className="mt-6 space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-2 px-3">
              <p className="text-[10px] tracking-wider font-bold text-gray-400 uppercase">NOTEBOOKS</p>
              <button
            ...
            <button
              ...
              style={{ color: theme === 'dark' ? '#71717a' : '#52525b', border: theme === 'dark' ? '1px dashed rgba(255,255,255,0.08)' : '1px dashed rgba(0,0,0,0.1)' }}
            >
              <Plus size={13} />
          ...
          <div>
            <div className="flex items-center justify-between mb-2 px-3">
              <p className="text-[10px] tracking-wider font-bold text-gray-400 uppercase">RECENT</p>
                ...
                {chatHistory.slice(0, 15).map((chat) => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2 pr-4">
                      <span className="text-xs font-medium line-clamp-1 text-gray-600 dark:text-zinc-400">{chat.title}</span>
                      <span className="text-[9px] shrink-0 text-gray-400 dark:text-zinc-650">{formatChatTime(chat.timestamp)}</span>
                    </div>
                    <button
        ...
        <div className="fixed inset-0 z-[999] flex items-center justify-center w-screen h-screen bg-black/60 backdrop-blur-md p-4 sm:p-6">
          <div className="relative w-full max-w-4xl h-[90vh] bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-between px-8 py-5 border-b border-zinc-100 dark:border-zinc-800">
              <div>
                <h2 className="text-lg font-bold text-zinc-900 dark:text-zinc-100">Create New Notebook</h2>
                <p className="text-xs text-zinc-500">Set up a personalized study workspace with custom icons and categories.</p>
              </div>
              <button 
